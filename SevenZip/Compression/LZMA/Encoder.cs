    ...
    public class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties
    {
        ...
        private class LiteralEncoder
        {
            public struct Encoder2
            {
                ...
                public void EncodeMatched(SevenZip.Compression.RangeCoder.Encoder rangeEncoder, byte matchByte,
                    byte symbol)
                {
                    uint num = 1u;
                    bool flag = true;
        ...
        private uint GetPosLenPrice(uint pos, uint len, uint posState)
        {
            ...
            uint num = (pos >= 128)
                ? (_posSlotPrices[(lenToPosState << 6) + GetPosSlot2(pos)] + _alignPrices[pos & 0xF])
                : _distancesPrices[lenToPosState * 128 + pos];
        ...
        private uint GetOptimum(uint position, out uint backRes)
        {
            ...
            _optimum[1].Price = _isMatch[(_state.Index << 4) + num5].GetPrice0() + _literalEncoder
                                    .GetSubCoder(position, _previousByte)
                                    .GetPrice(!_state.IsCharState(), indexByte2, indexByte);
            ...
            while (true)
            {
                ...
                uint num20 = price2 + _isMatch[(state.Index << 4) + num5].GetPrice0() + _literalEncoder
                                 .GetSubCoder(position, _matchFinder.GetIndexByte(-2))
                                 .GetPrice(!state.IsCharState(), indexByte2, indexByte);
                ...
                if (!flag && indexByte2 != indexByte)
                {
                    ...
                    if (matchLen >= 2)
                    {
                        ...
                        uint num23 = num20 + _isMatch[(state2.Index << 4) + num22].GetPrice1() +
                ...
                for (uint num27 = 0u; num27 < 4; num27++)
                {
                    ...
                    if (matchLen2 >= 2)
                    {
                        ...
                        uint num32 = num6 + GetRepPrice(num27, num28, state, num5) +
                                     _isMatch[(state3.Index << 4) + num31].GetPrice0() + _literalEncoder
                                         .GetSubCoder(position + num28,
                                             _matchFinder.GetIndexByte((int) (num28 - 1 - 1))).GetPrice(matchMode: true,
                                             _matchFinder.GetIndexByte((int) (num28 - 1 - (reps[num27] + 1))),
                                             _matchFinder.GetIndexByte((int) (num28 - 1)));
                        state3.UpdateChar();
                    ...
                    if (num38 == _matchDistances[num37])
                    {
                        if (num38 < val)
                        {
                            ...
                            if (matchLen3 >= 2)
                            {
                                ...
                                uint num42 = num40 + _isMatch[(state4.Index << 4) + num41].GetPrice0() + _literalEncoder
                                                 .GetSubCoder(position + num38,
                                                     _matchFinder.GetIndexByte((int) (num38 - 1 - 1)))
                                                 .GetPrice(matchMode: true,
                                                     _matchFinder.GetIndexByte((int) (num38 - (num39 + 1) - 1)),
                                                     _matchFinder.GetIndexByte((int) (num38 - 1)));
                                state4.UpdateChar();
        ...
        public void CodeOneBlock(out long inSize, out long outSize, out bool finished)
        {
            ...
            if (nowPos64 == 0)
            {
                ...
                uint num2 = (uint) ((int) nowPos64 & (int) _posStateMask);
            ...
            while (true)
            {
                ...
                uint num3 = (uint) ((int) nowPos64 & (int) _posStateMask);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (posSlot >= 4)
                        {
                            ...
                            if (posSlot < 14)
                            {
                                BitTreeEncoder.ReverseEncode(_posEncoders, num8 - posSlot - 1, _rangeEncoder, num7,
                                    num9);
                            }
                            else
                            {
        ...
        public void Code(Stream inStream, Stream outStream, long inSize, long outSize, ICodeProgress progress)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    CodeOneBlock(out long inSize2, out long outSize2, out bool finished);
                    if (finished)
                    {
                        break;
                    }
